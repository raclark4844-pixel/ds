...
export function IntakeForm({
  ...
  function next() {
    if (
      step === 1 &&
      (!selectedIndustries(brief.industry).length || !brief.name.trim() || !brief.email.trim())
    ) {
      setError("At least one industry, name, and email are required to continue.");
      return;
    }
    if (step === 2) {
  ...
  if (!hydrated)
    return (
      <div className="rounded-xl border border-line bg-surface p-6 text-muted">Loading brief…</div>
    );
    ...
    return (
      <div id="project-brief" className="rounded-xl border border-line bg-surface p-5 sm:p-8">
        ...
        <h2 className="mt-3 font-display text-3xl font-semibold tracking-tight">
          Your project brief was sent.
        </h2>
        <p className="mt-3 max-w-2xl text-muted">
          A copy remains saved in this browser. This is a project brief, not a contract, and it does
          not guarantee rankings, AI citations, traffic, lead volume, engagement, or conversion
          lifts.
        </p>
  ...
  return (
    ...
    >
      ...
      {brief.reportId ? (
        <p className="mt-4 rounded-lg border border-volt/40 bg-volt-dim px-4 py-3 text-sm text-volt">
          Continuing Demore Report ID <strong>{brief.reportId}</strong>. This same ID stays with the
          project brief, admin queue, CRM handoff and follow-up.
        </p>
      ) : null}
      <div
      ...
      >
        <div
          className="h-full bg-hot transition-[width] duration-300"
      ...
      <div className="mt-8 grid gap-5">
        {step === 1 ? (
          <>
            <IndustryMultiselect
              value={selectedIndustries(brief.industry)}
              onChange={(items) => patch({ industry: items.join(" | ") })}
            />
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <Field label="Role" htmlFor="role">
                <input
                  ...
                  placeholder="Owner, operator, marketing lead…"
                />
              </Field>
          ...
          <>
            <fieldset>
              <legend className="text-sm font-medium text-fg">
                What are you looking for? <span className="text-hot">*</span>
              </legend>
            ...
            <Field
              ...
              htmlFor="budget"
              required
              hint={`Minimum project budget is $${MIN_BUDGET.toLocaleString()}.`}
            >
            ...
            <Field
              ...
              htmlFor="goal"
              hint="What should be measurably better after this is built?"
            >
              <textarea
                id="goal"
                className={textarea}
            ...
            <Field label="Timeline" htmlFor="timeline">
              <input
                ...
                placeholder="Target launch date or business deadline"
              />
            </Field>
          </>
        ) : null}
        ...
        {step === 3 ? (
          <>
            <fieldset>
              <legend className="text-sm font-medium text-fg">Website / platform features</legend>
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
            ...
            <Field label="Must-have pages or functions" htmlFor="mustHavePages">
              <textarea
                id="mustHavePages"
                className={textarea}
            ...
            <Field label="Brand notes" htmlFor="brand">
              <textarea
                id="brand"
                className={textarea}
                value={brief.brand}
            ...
            <Field label="Sites you like and why" htmlFor="likedSites">
              <textarea
                id="likedSites"
                className={textarea}
            ...
            <Field label="Ideal customer / buyer" htmlFor="buyer">
              <textarea
                id="buyer"
                className={textarea}
                value={brief.buyer}
        ...
        {step === 4 ? (
          <>
            <fieldset>
              <legend className="text-sm font-medium text-fg">
                Platforms you use or want to use
              </legend>
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                {platforms.map((item) => (
                  <label
            ...
            <Field label="Desired posting frequency" htmlFor="frequency">
              <input
                id="frequency"
                className={control}
            ...
            <Field label="Who approves content" htmlFor="approver">
              <input
                id="approver"
                className={control}
            ...
            <Field label="Content and assets already available" htmlFor="existingContent">
              <textarea
                id="existingContent"
                className={textarea}
            ...
            <Field label="Current SEO / search / AI visibility" htmlFor="seoNow">
              <textarea
                id="seoNow"
                className={textarea}
                value={brief.seoNow}
        ...
        {step === 5 ? (
          <>
            <Field label="How do new leads reach you today?" htmlFor="leadProcess">
              <textarea
                id="leadProcess"
                className={textarea}
                value={brief.leadProcess}
                onChange={(e) => patch({ leadProcess: e.target.value })}
                placeholder="Phone, form, email, CRM, booking tool, social DMs…"
              />
            </Field>
            <Field label="Analytics / tracking in place today" htmlFor="analyticsNow">
              <textarea
                id="analyticsNow"
                className={textarea}
                value={brief.analyticsNow}
                onChange={(e) => patch({ analyticsNow: e.target.value })}
                placeholder="Google Analytics, Search Console, Meta Pixel, CRM tracking…"
              />
            </Field>
            <Field label="What repetitive work should be automated?" htmlFor="automationNeeds">
              <textarea
                id="automationNeeds"
                className={textarea}
                value={brief.automationNeeds}
                onChange={(e) => patch({ automationNeeds: e.target.value })}
              />
            </Field>
            <Field label="CRM and business tools you use" htmlFor="crmTools">
              <textarea
                id="crmTools"
                className={textarea}
                value={brief.crmTools}
                onChange={(e) => patch({ crmTools: e.target.value })}
              />
            </Field>
            ...
            <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-line bg-elevated px-4 py-3">
              ...
              <span className="text-sm leading-relaxed">
                I understand this is a project brief, not a contract, and results such as rankings,
                AI citations, traffic, lead volume, engagement, or conversion lifts are not
                guaranteed.
              </span>
            </label>
          </>
      ...
      <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-between">
        ...
        <Button type="submit" disabled={submitting}>
          {submitting ? "Sending…" : step < 5 ? "Continue" : "Submit project brief"}
        </Button>
      </div>
    </form>
  );
