  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between gap-3 text-xs text-muted">
        <span>{label}</span>
        <span className="font-medium tabular-nums text-fg">{total ? `${value}/${total}` : "Unavailable"}</span>
      </div>
...
function ReviewResult({ report, token }: { report: WebsiteReviewReport; token: string }) {
  ...
  async function download() {
    ...
    try {
      ...
      const name = res.headers.get("content-disposition")?.match(/filename="([^"]+)"/)?.[1] || `Demore-Website-Report-${report.recordId}.pdf`;
    ...
    <section className="review-result mt-8 space-y-6" aria-label="Website review results">
      <div>
        ...
        <p className="mt-3 text-sm text-muted">
          Compare observable website features with Demore Exterior Solutions and plan the next improvements.
        </p>
      </div>

      <div className="flex flex-wrap gap-5 text-xs text-muted">
        <span className="inline-flex items-center gap-2"><span className="size-2.5 rounded-sm bg-volt" />Your page</span>
        <span className="inline-flex items-center gap-2"><span className="size-2.5 rounded-sm bg-muted" />Demore Exterior Solutions</span>
      </div>

      <div className="space-y-5">
        {report.categories.map((category) => {
          const refs = report.benchmark.checks.filter((row) => row.category === category.name && row.status !== "Not applicable" && row.status !== "Unavailable");
          ...
          return (
            <div key={category.name} className="space-y-2">
              ...
              <Bar value={count} total={refTotal} tone="reference" label="Demore Exterior Solutions" />
            </div>
          );
        })}
      </div>
      ...
      <div className="overflow-x-auto rounded-md border border-line">
        <table className="w-full min-w-[640px] text-left text-sm">
          <caption className="px-3 py-2 text-left text-xs font-medium text-muted">Current page compared with Demore Exterior Solutions</caption>
            ...
            {report.current.checks.map((item) => {
              ...
              return (
                <tr key={item.id} className="border-t border-line">
                  <th scope="row" className="p-3 font-medium text-fg">{item.label}</th>
                  <td className="py-3"><span className={statusClass(item.status)}>{item.status}</span></td>
                  <td className="py-3"><span className={statusClass(reference)}>{reference}</span></td>
      ...
      <div>
        ...
        {report.recommendations.length ? (
          ...
          <p className="mt-3 text-sm text-muted">All applicable signals were detected. Next, test the full inquiry journey and review quality, accessibility, and measurement.</p>
        )}
      </div>

      <div>
        <h3 className="font-display text-xl font-semibold">Beyond the Exterior Solutions benchmark</h3>
        <p className="mt-2 text-sm text-muted">
          These Demore Technology Solutions recommendations are tailored to your industry, whether or not the reference site uses them. These are opportunities to scope, not confirmed installations or included deliverables.
        </p>
        <div className="mt-4 space-y-3">
          {report.offerings.map(([name, detail]) => (
            <p key={name} className="text-sm"><strong>{name}.</strong> {detail}</p>
          ))}
        </div>
        <p className="mt-4 text-xs text-muted">{report.methodology}</p>
      </div>

      <p className="text-sm text-muted">Record ID {report.recordId} — same number as Ask Demore and the project form.</p>
      <p className="text-xs text-muted">Rankings, AI citations, and conversion lifts are not guaranteed.</p>
...
export function WebsiteReview() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/website-review", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ url, industry, record_id: recordId || getRecordId() }),
      ...
      setError(name === "TimeoutError" ? "The review timed out. Please try again." : err instanceof Error ? err.message : "Unable to complete the review.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <section id="website-review" className="review-shell mt-16 scroll-mt-24">
      ...
      <h2 className="mt-3 max-w-3xl font-display text-3xl font-semibold tracking-tight sm:text-4xl">See what your website could do next.</h2>
      <p className="mt-4 max-w-2xl text-muted">Enter a public website address for a side-by-side review, practical recommendations, and a professionally branded PDF.</p>
      ...
      <form onSubmit={submit} className="mt-8 space-y-4">
        ...
        <p id="industry-help" className="text-xs text-muted">Leave blank to search your website for an industry suggestion. Enter or correct it here and run the review again.</p>
        <label className="block text-sm font-medium" htmlFor="audit-url">
          ...
          <div className="mt-1 flex flex-col gap-3 sm:flex-row">
            <input
              ...
              autoComplete="url"
            />
            <Button type="submit" variant="volt" disabled={busy}>{busy ? "Reviewing website…" : "Review my website"}</Button>
          </div>
        </label>
      </form>
