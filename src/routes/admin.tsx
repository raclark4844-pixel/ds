...
function ComparisonCard({ item, disabled, onChange, onSave }: { item: QueueItem; disabled: boolean; onChange: (item: QueueItem) => void; onSave: (item: QueueItem) => Promise<void> }) {
  ...
  return (
    <article className="rounded-xl border border-line bg-surface p-5 sm:p-6">
      ...
      <div className="mt-3 rounded-md border border-line bg-bg p-3 text-sm"><strong>Customer handoff: {item.handoffStatus}</strong>{item.handoff ? <p className="mt-1 text-muted">Same record {item.id} · project contact {String(item.handoff.name || report.contactName)} · timeline {String(item.handoff.timeline || report.timeframe || "not provided")}</p> : <p className="mt-1 text-muted">No project brief has been attached to this report yet.</p>}</div>
      <div className="mt-5 overflow-x-auto"><table className="w-full min-w-[720px] text-left text-xs"><thead className="text-muted"><tr><th className="pb-2">Competitor</th><th>Source</th><th>Maps</th><th>Organic</th><th>Rating</th><th>Est. traffic/mo.</th><th>Website score</th></tr></thead><tbody>{report.competitors.map((row) => <tr key={`${row.name}-${row.website}`} className="border-t border-line"><td className="py-2 pr-3 text-fg">{row.name}</td><td>{row.source || row.evidence}</td><td>{row.mapsRank ? `#${row.mapsRank}` : "—"}</td><td>{row.organicRank ? `#${row.organicRank}` : "—"}</td><td>{row.rating ? `${row.rating} (${row.reviewCount ?? 0})` : "—"}</td><td>{row.estimatedMonthlyOrganicTraffic?.toLocaleString() ?? "—"}</td><td>{row.total}</td></tr>)}</tbody></table></div>
