...
export async function handleWebsiteReviewPdf(req: Request) {
  ...
  if (!rateLimit(`review-pdf:${ip}`, 12))
    return Response.json({ error: "Too many download requests." }, { status: 429 });
  let raw: unknown;
  try {
    raw = await req.json();
  } catch {
  ...
  try {
    ...
    console.error(
      "[website-review-report] pdf failed",
      err instanceof Error ? err.message : "unknown",
    );
    return Response.json(
      { error: "The PDF could not be generated. Retry without losing the review." },
      { status: 500 },
    );
  }
}
